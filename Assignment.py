...
class CSP:
    ...
    def add_variable(self, name: str, domain: list):
        ...
        self.domains[name] = list(domain)       #domains is a list that belongs to each variable. 
    ...
    def filterfunction_equal(self,i,j):
        return i!=j
    
    def filterfunction_greater(self,i,j):
        return i>j
    
    def filterfunction_lesser(self,i,j):
        return i<j
    ...
    def select_unassigned_variable(self, assignment):
        ...
        of legal values has a length greater than one. (Unsolved variable) I interpret assignment as a list of variables here
        """
        
        for i in assignment:                                                    #go through variables (i) in assignment
            numberofdomains=0
            for j in self.domains[i]:                                           #go through domains (j) in variable (i)
                numberofdomains+=1
                if(numberofdomains>1):
                    return i

    def inference(self, assignment, queue):                                     #assignment is for example A=[1,2,], B=[1,2,3]. self.domain[i]? queue is initially all arcs in csp
        """The function 'AC-3' from the pseudocode in the textbook.             #checks inconsistency is found
        'assignment' is the current partial assignment, that contains
        the lists of legal values for each undecided variable. 'queue'
        is the initial queue of arcs that should be visited.
        This is the actual algorithm, or how to solve it. 
        """
        #queue will be all arcs of csp. For example (A,B),(B,A),(C,A)

        
        queue=self.get_all_arcs                                                 #queue is initialy all arcs
        while(len(queue)!=0):
            tempfirstinqueue=queue[0]                                           #temp save for first in queue
            del queue[0]                                                        #removes first arc
            if self.revise(tempfirstinqueue):                                   #(xi, xj) is an arc
                if(len(self.domains[len(tempfirstinqueue[0])])==0):                  #if domain of first variable is zero elements
                    return False
                for variables in self.get_all_neighboring_arcs(x1):
                    queue.append[xk,xi]
        return True
        

    def revise(self, assignment, i, j):
        """The function 'Revise' from the pseudocode in the textbook.           #assignment is a list of assignments, that contains possible values for 
        'assignment' is the current partial assignment, that contains           undecided variable. 
        ...
        for x in self.domain[i]:
            if(....)
                self.domain[i].delete(x)
                revised=True
        return revised
'''
    def order_domain_values(self, variable):
        '''Order domain value of a variable in object'''
        self.domains[variable]=sorted(self.domains[variable])                        #sort domains of variable
...
print(csp.domains)
print("")
csp.add_constraint_one_way('A', 'B', csp.filterfunction_greater)        #A has to be greater than B. A is the first in pair. 
csp.add_constraint_one_way('B','A', csp.filterfunction_lesser)          #same, but opposite


print("")
print(csp.constraints)                                                  #prints legal value pairs 
print("")
print(csp.get_all_arcs())
print("")
print("Arcs to/from variable", csp.get_all_neighboring_arcs('A'))                                #get arcs to/from variable